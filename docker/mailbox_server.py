...
# Example p4p server code from Matt Gibbs
...
float_value = SharedPV(nt=NTScalar('f'), initial=0.0, handler=Handler())
...
# kill it.
pva_server = PVAServer.forever(providers=[{f"JESSE:LIVE:TWISS": live_twiss_pv,
					   f"JESSE:LIVE:FLOAT": float_value,
					   f"JESSE:LIVE:RMAT": live_rmat_pv}])
